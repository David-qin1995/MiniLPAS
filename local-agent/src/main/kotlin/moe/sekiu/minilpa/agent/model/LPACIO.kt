...
private data class LPACOutDelegate(val type : LPACIO.Type, val payload : JsonElement) {
    constructor(lpacio : LPACIO) : this(
        lpacio.type,
        JsonObject(
            json.encodeToJsonElement(lpacio.payload).jsonObject.toMutableMap().apply { remove("type") }
        )
    )
}
...
class LPAOperationFailureException(val lpa : LPACIO.Payload.LPA) : RuntimeException(lpa.message) {
    val data = lpa.data
}
